...
class DrQA(object):
    ...
    def __init__(
        ...
        reader_model = "./DATA_DIR/reader_models/multitask.mdl" #reader_model or DEFAULTS['reader_model']
        ...
        #)
        init(tok_class, tok_opts, db_class, db_opts,fixed_candidates)
    def _split_doc(self, doc):
        """Given a doc, split it into chunks (by paragraph)."""
        curr = []
        curr_len = 0
        #x = doc
        #print('printing to check the document doc',doc,'---------',x)
        for split in regex.split(r'\n+', doc):
            #print('printing slipt....',split)
            split = split.strip()
            if len(split) == 0:
                continue
    ...
    def process(self, query, candidates=None, top_n=1, n_docs=5,
        ...
        #print('inside process',predictions[0])
    ...
    def process_batch(self, queries, candidates=None, top_n=1, n_docs=5,
        ...
        logger.info('printing the query.....%s'%queries)
        logger.info('Retrieving top %d docs...' % n_docs)

        logger.info('printing the query....%s'%queries[0])

        # Rank documents for queries.
        if len(queries) == 1:
            ranked = [self.ranker.closest_docs(queries[0], k=n_docs)]
            logger.info('searching web for relavent sites__________')
            links = list(search(str(queries[0]),tbs="qdr:h",stop = 1,user_agent='shravan'))
            #links = list(search(str(queries[0]),pause = 20,stop = 1,user_agent='shravan'))
            #links = list(search(str(queries[0]),stop=1))
        ...
        logger.info(all_docids, all_doc_scores)
        # Flatten document ids and retrieve text from database.
        # We remove duplicates for processing efficiency.
        flat_docids = list({d for docids in all_docids for d in docids})
        #logger.info('printing flat_docids.....%s'%flat_docids)
        did2didx = {did: didx for didx, did in enumerate(flat_docids)}
        #logger.info('printing did2didx.....%s'%did2didx)
        doc_texts = [fetch_text(i) for i in flat_docids] #self.processes.map(fetch_text, flat_docids)
        #logger.info('printing the doc texts........ %s'%doc_texts)
        ...
        for text in doc_texts:
            #logger.info('printing the text .... %s'%text)
        ...
        q_tokens = [tokenize_text(j) for j in queries]#self.processes.map_async(tokenize_text, queries)
        s_tokens = [tokenize_text(h) for h in flat_splits]#self.processes.map_async(tokenize_text, flat_splits)
        ...
        for queue in queues:
            ...
            predictions = []
            while len(queue) > 0:
                #print('queue_________________',queue)
                
                #print('printing links________',links)
